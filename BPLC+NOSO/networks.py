...
from functions import xav_uniform_rev_
...
class FMNISTnet(nn.Module):
    ...
    def forward(self, input, batch_size, mode):
        ...
        for step in range(self.num_steps): # simulation time steps       
            ...
            hc1_vm, hc1_vs, hc1_um, hc1_us, hc1_spike = noso(self.thresh, 
                                                             self.tau_m, 
                                                             self.tau_s, 
                                                             self.conv1, 
                                                             in_spike, 
            ...
            # Update refractory filter
            with torch.no_grad() : 
                hc1_sav -= hc1_spike.clone()
            ...
            hc2_vm, hc2_vs, hc2_um, hc2_us, hc2_spike = noso(self.thresh, 
                                                             self.tau_m, 
                                                             self.tau_s, 
                                                             self.conv2, 
            ...
            hf1_vm, hf1_vs, hf1_um, hf1_us, hf1_spike = noso(self.thresh, 
                                                             self.tau_m, 
                                                             self.tau_s, 
                                                             self.fc1, 
            ...
            # Update refractory filter
            with torch.no_grad() : 
                hf1_sav -= hf1_spike.clone()
            
            # Calculation of second linear layer
            hf2_vm, hf2_vs, hf2_um, hf2_us, hf2_u, hf2_spike = noso(self.thresh, 
                                                                    self.tau_m, 
                                                                    self.tau_s, 
                                                                    self.fc2, 
                                                                    hf1_spike, 
...
class CIFARnet(nn.Module):
    def __init__(self, task, thresh, tau_m, tau_s, num_steps, scaling_factor):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d): 
                nn.init.xavier_uniform_(m.weight.data)
            if isinstance(m, nn.Linear): 
                xav_uniform_rev_(m.weight.data, gain=3)
    ...
    def forward(self, input, batch_size, mode):
        ...
        for step in range(self.num_steps): # simulation time steps               
            ...
            hc3_vm, hc3_vs, hc3_um, hc3_us, hc3_spike = noso(self.thresh, 
                                                             self.tau_m, 
                                                             self.tau_s, 
                                                             self.conv3, 
            ...
            hc4_vm, hc4_vs, hc4_um, hc4_us, hc4_spike = noso(self.thresh, 
                                                             self.tau_m, 
                                                             self.tau_s, 
                                                             self.conv4, 
            ...
            # Update refractory filter
            with torch.no_grad() : 
                hc4_sav -= hc4_spike.clone()
            
            
            # Calculation of fifth convolutional layer
            hc5_vm, hc5_vs, hc5_um, hc5_us, hc5_spike = noso(self.thresh, 
                                                             self.tau_m, 
                                                             self.tau_s, 
                                                             self.conv5, 
                                                             hc4_spike,
            ...
            # Calculation of second linear layer
            hf2_vm, hf2_vs, hf2_um, hf2_us, hf2_spike = noso(self.thresh, 
                                                             self.tau_m, 
                                                             self.tau_s, 
                                                             self.fc2, 
                                                             hf1_spike, 
            ...
            hf3_vm, hf3_vs, hf3_um, hf3_us, hf3_u, hf3_spike = noso(self.thresh, 
                                                                    self.tau_m, 
                                                                    self.tau_s, 
                                                                    self.fc3, 
                                                                    hf2_spike, 
