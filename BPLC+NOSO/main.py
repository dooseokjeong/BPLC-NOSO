...
from functions import lr_warmup
...
parser.add_argument('--task', type=str, default='CIFAR10', help='which task to run (FMNIST or CIFAR10)')
parser.add_argument('--network', type=str, default='CIFARnet', help='which network to run (FMNISTnet or CIFARnet)')
...
parser.add_argument('--tau_m', type=float, default=165, help='Time constant of membrane potential kernel [ms]')
...
parser.add_argument('--weight_decay', type=float, default=1E-3, help='Weight decay (L2 regularization) coefficient')
...
def main():
    ...
    if args.mode == 'train':
        ...
        for epoch in range(args.num_epochs):
            # lr scheduler step and lr warm up for CIFAR10
            if epoch != 0 :
                scheduler_step(optimizer, epoch, args.lr_decay_interval, args.lr_decay_rate)
                if args.task == 'CIFAR10':
                    lr_warmup(optimizer, epoch, alpha=1E-2, end_lr=5E-2)
...
def train(model, train_loader, criterion, optimizer):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        # If all output neurons dead, stop code
        if outputs[2][-1] == 0 :
            sys.exit("All output neurons dead, training stop")
        ...
        if args.task == 'FMNIST':
            ...
            train_set_len = 5000
        if (i*10) % (train_set_len // args.batch_size) == 0 : 
            print("=== {} / {} updating...".format(i, len(train_loader)))
            print("spike count = {}".format(outputs[2]))
            print("output latency = {}".format(torch.unique(outputs[0])))
...
def test(model, test_loader, criterion):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            ...
            # If multi output neurons has same latency, compare the membrane potential of output neurons when spiking
